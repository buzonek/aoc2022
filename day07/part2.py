...
def nodes_with_size_greater_than(node: Node, limit: int) -> list[Node]:
    nodes = []
    if not node.is_file and node.size > limit:
        nodes.append(node)
    for child in node.childs:
        _nodes = nodes_with_size_greater_than(child, limit)
        nodes.extend(_nodes)
    return nodes
...
def compute(s: str) -> int:
    ...
    size_needed = 30000000 - (70000000 - tree.root.size)
    nodes = nodes_with_size_greater_than(tree.root, limit=size_needed)
    return min(nodes, key=lambda x: x.size).size
